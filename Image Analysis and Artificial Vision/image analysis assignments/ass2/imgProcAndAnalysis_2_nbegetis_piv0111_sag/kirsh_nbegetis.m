...
G=uint8(G);
imshow(G);
imwrite(G,'./Output_images/kirsh_AIRPORT_without_threshold.tif'); 
...
while threshold < 1 || threshold > 256 
    threshold = input('Give a threshold to be used with Kirsh filtering (1-256):  ');
end;
...
H=uint8(H);
imshow(H);
imwrite(H,'./Output_images/kirsh_AIRPORT_with_threshold.tif'); 
