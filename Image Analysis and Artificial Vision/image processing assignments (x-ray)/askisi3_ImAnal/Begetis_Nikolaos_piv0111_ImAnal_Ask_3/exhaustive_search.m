function [] = exhaustive_search(evaluation_method, cl, superClass, classLabels, patterns1, patterns2)   
    patternsInBothClasses = size(superClass, 1);
	first_time = zeros(9, 1);
    temp = superClass;
    clear superClass; superClass = [];
    I = rankfeatures(temp', classLabels, 'CRITERION', 'wilcoxon');
    I = sort(I(1 : 12),'ascend')
    for i = 1 : length(I)
        pattern = temp(:, I(i));
    ...
    tStart(cl) = tic;
    success = 0;
    for i = 1 : 5 %5 epanalipseis wste na petixoume mexri 5 dinatous sindiasmous xarakthristikwn
        ...
        for j = 1 : size(fc, 1)
            ...
            for k = 1 : size(superClass, 2) %epanalamvanoume 12 fores gia na vroume poia xaraktiristika prepei na afisoume ektos
