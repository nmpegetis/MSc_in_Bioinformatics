...
my @Folders = ("Benchmark_2","Benchmark_3_updated","Benchmark_4_updated") ;
my $rootFolder = "/home/mitsias/Intervor/PDB_files/benchmark4/structures/";

for (my $i = 0;$i<scalar(@Folders);$i++){
    print $Folders[$i],"\n";
    my $Benchmark =  $rootFolder.$Folders[$i]."/*.pdb";
    my @Pdbs = glob $Benchmark;
    for (my $j = 0;$j<scalar(@Pdbs);$j++){
      #print $Pdbs[$j],"\n";
      my $justPDB;
      if ($Pdbs[$j] =~ m/\/(\w\w\w\w_\w_\w).pdb/){
	  $justPDB = $1;
	  #print $justPDB,"\n";
      }
      my $PDBFolder = $rootFolder."/".$Folders[$i]."/".$justPDB;
      system "mkdir $PDBFolder";
#     open (File,"<",$rootFolder.$Folders[$i]."/Pdbs.txt") or die "$!\n";
#     while (my $line = <File>){
...
#       my $result=`wget -O $TempFileName "www.rcsb.org/pdb/download/downloadFile.do?fileFormat=pdb&compression=NO&structureId=$line"`;
       my $TempFile = $Pdbs[$j];
       my $result = `/home/mitsias/Intervor/./intervor-static-32.exe -f $TempFile -C A -C B -w 1 -p all -D $PDBFolder --log`
# #       getstore($url, $file);
#     }
    }
}
